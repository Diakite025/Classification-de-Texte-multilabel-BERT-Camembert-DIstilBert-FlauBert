...
def Undersampling(df,valeur):
    ...
    categorie_a_aumenter=["drinking_water","paper_and_cardboard","hydraulic_equipment"]
    count=valeur
...
def traduire_texte(texte, source_lang='en', target_lang='fr'):
    ...
    translator_en_fr = GoogleTranslator(source=source_lang, target=target_lang)
    translator_fr_en = GoogleTranslator(source=target_lang, target=source_lang)
    
    if len(texte) < 4000:
       
   
        traduction_en_fr = translator_en_fr.translate(texte)
        traduction_fr_en = translator_fr_en.translate(traduction_en_fr)
    ...
    return traduction_fr_en





def Oversampling(data:pd.DataFrame,count, collection: str,lien_BD_MongoDB,name_categories=utility.Liste_categories(), source_lang='en', target_lang='fr'):
    
    """
    Description : Cette fonction effectue le sur-échantillonnage des catégories moins représentées.
