...
def nb_exemple_categorie(df):
    count_exemple_df = pd.DataFrame(columns=["Label", "Nombre_de_textes"])


    for label in df.columns:
        count = df[df[label] == 1].shape[0]  
        count_exemple_df = count_exemple_df.append({"Label": label, "Nombre_de_textes": count}, ignore_index=True)
